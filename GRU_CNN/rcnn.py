...
with tf.name_scope("decode"):
    ...
    with tf.variable_scope("answer"):
        forward_cell = tc.rnn.GRUCell(embedding_size, kernel_initializer=he_init)
        forward_cell_drop = tc.rnn.DropoutWrapper(
            forward_cell, input_keep_prob=keep_prob)
        backward_cell = tc.rnn.GRUCell(embedding_size, kernel_initializer=he_init)
        backward_cell_drop = tc.rnn.DropoutWrapper(
            backward_cell, input_keep_prob=keep_prob)
        bi_answer_outputs, bi_answer_state = tf.nn.bidirectional_dynamic_rnn(
            forward_cell_drop, backward_cell_drop, answer_emb, sequence_length=answer_sequence_length, time_major=True, dtype=tf.float32)
        cl, cr = bi_answer_outputs
        new_embeddings = tf.concat([cl,answer_emb,cr],-1)
        new_embeddings = tf.transpose(new_embeddings,(1,0,2))
        dense = tf.layers.dense(new_embeddings,num_units)
        pool = tf.reduce_max(dense,1)
with tf.name_scope("logits"):
    logits = tf.layers.dense(pool, n_outputs)
    labels = simi_values
