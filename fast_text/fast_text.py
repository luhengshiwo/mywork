...
with tf.name_scope("decode"):
    ...
    with tf.variable_scope("answer"):
        answer_state = tf.reduce_mean(answer_emb, axis=1)
        answer_state_drop = tf.nn.dropout(answer_state, keep_prob=train_keep_prob)

with tf.name_scope("logits"):
    logits = tf.layers.dense(answer_state_drop, n_outputs)
    labels = simi_values
