...
def pad_sentence_batch(sentences):
    ...
    for sentence in sentences:
        if len(sentence)<max_sentence:
            newsentences.append(sentence + [pad_int]* (max_sentence - len(sentence)))
        else:
            newsentences.append(sentence[0:max_sentence])
    return newsentences
...
def generate_batch(source_path, batch_size, is_train=True, shuffle=False):
    ...
    while True:
        ...
        batch_count += 1
        first_batch_pad = pad_sentence_batch(
            first[start:end])
        second_batch_pad = pad_sentence_batch(
            second[start:end])
        if is_train:
            simi_batch = simi[start:end]
            yield (first_batch_pad, second_batch_pad,simi_batch)
        else:
            ids_batch = ids[start:end]
            yield (ids_batch,first_batch_pad, second_batch_pad)
...
def generate_batch_shell(source_sentence, batch_size=batch_size):
    ...
    second = []
    simi = []
    lines = source_sentence.strip().split('\t')
    jieba.load_userdict(mydict)
    for _ in range(batch_size):
        # newline0 = " ".join(jieba.cut(lines[0]))
        # newline1 = " ".join(jieba.cut(lines[1]))
        newline0 = " ".join(list(lines[0]))
        newline1 = " ".join(list(lines[1]))
    ...
    while True:
        ...
        second_batch_pad= pad_sentence_batch(
            second[start:end])
        yield (first_batch_pad,second_batch_pad)
